...
def test_json_base():
    ...
    assert a.to_json() == {"type_gref": "test_types:A", "x": 1, "y": "hello"}
    assert b.to_json() == {"type_gref": "test_types:B", "x": 2, "y": "world", "z": 3.14}
    ...
    assert B.from_json({"x": 1, "y": "hello"}).to_json() == {
        "type_gref": "test_types:B",
        "x": 1,
        "y": "hello",
        "z": 0.0,
    }
    assert A.from_json({"x": 1, "y": "hello"}).to_json() == {
        "type_gref": "test_types:A",
        "x": 1,
        "y": "hello",
    }
