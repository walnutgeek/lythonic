...
- [0001 Exposure matrices are immutable; all growth happens in a
  builder](../adr/0001-immutable-matrix-with-builder.md) — the matrix has no
  setters at all; `ExposureMatrixBuilder` is the only mutable object.
- [0002 Exposure records serialize as index
  triples](../adr/0002-exposure-records-as-index-triples.md) — records persist
  as `(subject index, target index, value)`; indexes never reach the interface.

Closed tickets:

- #7 Sparse keyed exposure matrix (Universe, ExposureMatrix,
  ExposureMatrixBuilder)
- #8 Retrofit FrameData to the pd/pl/pa facade pattern (breaking)
